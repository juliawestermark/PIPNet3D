    ...
    for k, (xs, ys) in img_iter: # shuffle is false so should lead to same order as in imgs
        ...
        with torch.no_grad():
            ...
            for pred_class_idx in sorted_out_indices:
                ...
                sorted_pooled, sorted_pooled_indices = torch.sort(pooled.squeeze(0), descending=True) # (num_ps) -> prototypes sorted according their detection
...
def check_empty_prototypes(args, net, img_prototype_top1, proto_coord_top1):
    ...
    for p in tqdm(range(net.module._num_prototypes), desc="Check prototypes"):
        ...
        if img_prototype_top1[p]:
            ...
            prototype = img_tensor[0, 0, d_min:d_max, h_min:h_max, w_min:w_max]
            histogram, _ = np.histogram(prototype.numpy(), bins=256, density=True)
            ps_entropy = entropy(histogram, base=2)
